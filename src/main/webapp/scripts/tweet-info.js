...
function tweetInfoWhoCanReplyChoiceClikced(choice, tweet_id) {
    ...
    // $('#top-layer-tweet-who-can-reply-string').text(whoCanReplyStrings[choice] + ' can reply');
    ...
    return `
        <div style="margin-bottom: 20px;">
            ...
            <div style="display: block;">
                ...
                <div style="display: flex; margin: 0 10px; margin-top: 20px; color: rgb(110, 110, 110); font-weight: 450; align-items: center;">
                    <div>
                        <span>${dateTime.time}</span>
                        <span>·</span>
                        <span>${dateTime.date}</span>
                        <span>·</span>
                        <span>${sourceLabels[tweetbox['tweet']['source_label']]}</span>
                    </div>
                ...
                <hr style="margin-top: 15px; margin-bottom: 15px; height:1px; border:none; color:#333; background-color:#333;">
                ...
                <hr style="margin-top: 0px; margin-bottom: 15px; height:1px; border:none; color:#333; background-color:#333;">
...
function getRQTweetInfo(tweet_id, like_count) {
    ...
    $.ajax({
        ...
        success: function(rq) {
            ...
            ans = `
                <div>
                    <div style="display: flex; align-items: center; margin: -2px 10px;">
                        ${rq[0] == 0 ? `` : `
                            <div onclick="showWhoRetweeted(${tweet_id})" style="margin-right: 20px; cursor: pointer;">
                                <span style="font-size: 14px; font-weight: 600;">${rq[0]}</span> <span style="font-size: 13px; color: rgb(107, 104, 99);">Retweet${rq[0] == 1 ? `` : `s`}</span>
                            </div>
                        `}

                        ${rq[1] == 0 ? `` : `
                            <div onclick="showQuotedTweets(${tweet_id})" style="margin-right: 20px; cursor: pointer;">
                                <span style="font-size: 14px; font-weight: 600;">${rq[1]}</span> <span style="font-size: 13px; color: rgb(107, 104, 99);">Quote${rq[1] == 1 ? `` : `s`}</span>
                            </div>
                        `}

                        ${like_count == 0 ? `` : `
                            <div onclick="showWhoLiked(${tweet_id})" style="cursor: pointer;">
                                <span style="font-size: 14px; font-weight: 600;">${like_count}</span> <span style="font-size: 13px; color: rgb(107, 104, 99);">Like${like_count == 1 ? `` : `s`}</span>
                            </div>
                        `}
                    </div>
                    <hr style="margin-top: 15px; margin-bottom: 15px; height:1px; border:none; color:#333; background-color:#333;">
